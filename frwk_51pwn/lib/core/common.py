...
from frwk_51pwn.lib.core.decorators import cachedmethod
from frwk_51pwn.lib.core.enums import OS_ARCH, OS
from frwk_51pwn.lib.core.exception import frwk_51pwnSystemException
from frwk_51pwn.lib.core.log import LOGGER_HANDLER
from frwk_51pwn.lib.core.settings import (
...
from frwk_51pwn.lib.core.settings import IPV6_ADDRESS_REGEX
from frwk_51pwn.lib.core.settings import IP_ADDRESS_REGEX
from frwk_51pwn.lib.core.settings import OLD_VERSION_CHARACTER
from frwk_51pwn.lib.core.settings import POCSUITE_VERSION_CHARACTER
from frwk_51pwn.lib.core.settings import POC_NAME_REGEX
from frwk_51pwn.lib.core.settings import POC_REQUIRES_REGEX
from frwk_51pwn.lib.core.settings import UNICODE_ENCODING
from frwk_51pwn.lib.core.settings import URL_ADDRESS_REGEX
...
def check_file(filename):
    ...
    if not valid:
        raise frwk_51pwnSystemException("unable to read file '%s'" % filename)
    return valid
...
def set_paths(root_path):
    ...
    _ = os.path.join(paths.POCSUITE_HOME_PATH, ".frwk_51pwn")
    ...
    paths.POCSUITE_SHELL_HISTORY = os.path.join(_, "frwk_51pwn.hst")
    paths.POCSUITE_CONSOLE_HISTORY = os.path.join(_, "console.hst")

    paths.POCSUITE_TMP_PATH = os.path.join(_, "tmp")
    paths.POCSUITE_RC_PATH = os.path.join(paths.POCSUITE_HOME_PATH, ".frwk_51pwnrc")
...
def banner():
    """
    Function prints frwk_51pwn banner with its version
    """
    _ = BANNER
...
def get_latest_revision():
    ...
    ret = None
    resp = requests.get(url="https://raw.githubusercontent.com/hktalent/51pwn-framework/master/frwk_51pwn/__init__.py")
    try:
...
def is_frwk_51pwn_poc(poc_string):
...
def is_frwk_51pwn_poc(poc_string):
    return True if "frwk_51pwn" in poc_string else False
...
def get_file_items(filename, comment_prefix='#', unicode_=True, lowercase=False, unique=False):
    ...
    try:
        ...
        raise frwk_51pwnSystemException(err_msg)
