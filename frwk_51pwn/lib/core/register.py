...
from frwk_51pwn.lib.core.common import (
    multiple_replace, get_filename, get_md5,
    is_frwk_51pwn_poc, get_poc_requires, get_poc_name)
from frwk_51pwn.lib.core.data import kb
from frwk_51pwn.lib.core.data import logger
from frwk_51pwn.lib.core.settings import POC_IMPORTDICT
...
class PocLoader(Loader):
    ...
    def get_data(self, filename):
        if filename.startswith('frwk_51pwn://') and self.data:
            if not is_frwk_51pwn_poc(self.data):
...
def load_string_to_module(code_string, fullname=None):
    try:
        ...
        file_path = 'frwk_51pwn://{0}'.format(module_name)
