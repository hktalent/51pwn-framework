...
from frwk_51pwn.lib.core.exception import frwk_51pwnConnectionException
from frwk_51pwn.lib.core.exception import frwk_51pwnThreadException
from frwk_51pwn.lib.core.exception import frwk_51pwnUserQuitException
from frwk_51pwn.lib.core.exception import frwk_51pwnValueException
from frwk_51pwn.lib.core.settings import MAX_NUMBER_OF_THREADS
...
def run_threads(num_threads, thread_function, args: tuple = (), forward_exception=True, start_msg=True):
    ...
    except (KeyboardInterrupt, frwk_51pwnUserQuitException) as ex:
        kb.thread_continue = False
        kb.thread_exception = True
    ...
    except (frwk_51pwnConnectionException, frwk_51pwnValueException) as ex:
        kb.thread_exception = True
        logger.error("thread {0}: {1}".format(threading.currentThread().getName(), str(ex)))
        if conf.verbose > 1:
            traceback.print_exc()
