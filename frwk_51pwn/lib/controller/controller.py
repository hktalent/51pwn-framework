...
from frwk_51pwn.lib.core.common import data_to_stdout, desensitization
...
from frwk_51pwn.lib.core.exception import frwk_51pwnValidationException, frwk_51pwnSystemException
from frwk_51pwn.lib.core.poc import Output
from frwk_51pwn.lib.core.settings import CMD_PARSE_WHITELIST
from frwk_51pwn.lib.core.threads import run_threads
from frwk_51pwn.modules.listener import handle_listener_connection
from frwk_51pwn.modules.listener.reverse_tcp import handle_listener_connection_for_console
...
def runtime_check():
    if not kb.registered_pocs:
        ...
        raise frwk_51pwnSystemException(error_msg)
...
def start():
    ...
    logger.debug("frwk_51pwn will open {} threads".format(conf.threads))
    ...
    if conf.mode == "shell" and not conf.api:
        ...
        logger.info(info_msg)
        info_msg = "watting for shell connect to frwk_51pwn"
        logger.info(info_msg)
...
def task_run():
    while not kb.task_queue.empty() and kb.thread_continue:
        ...
        if conf.pcap:
            ...
            from frwk_51pwn.lib.utils.pcap_sniffer import Sniffer
        ...
        if hasattr(poc_module, "_options"):
            ...
            for opt, v in poc_module.options.items():
                ...
                if opt in CMD_PARSE_WHITELIST:
                    info_msg = "Poc:'{0}' You can't customize this variable '{1}' because it is already taken up by the frwk_51pwn.".format(
                        poc_name, opt)
        ...
        except frwk_51pwnValidationException as ex:
            info_msg = "Poc:'{}' frwk_51pwnValidationException:{}".format(poc_name, ex)
            logger.error(info_msg)
            result = None
